...
var port = chrome.runtime.connect({ name: 'Timed' }); // start a long-lived connection with background for time tracking
...
port.onMessage.addListener(function (msg) {
  switch (msg.action) {
    case 'UPDATE_TRACKED_TIME':
      ...
      if (msg.trackObject._id) window.postMessage(msg, 'http://localhost:3000');
  }
});

...
var idleTimer = void 0; // the 5s timer that checks idleness of user
var isTrackingTime = false; // variable to check if app is currently tracking time spent on facebook
var trackingTimePort = chrome.runtime.connect({ name: 'https://facebook.com' }); // start a long-lived connection with background for time tracking
var timeLabel = void 0; // DOM element that contains the time spent on facebook
 ...
 */
trackingTimePort.onMessage.addListener(function (msg) {
  if (msg.action == 'UPDATE_TRACKED_TIME') updateLabel(msg.trackObject.today);
});
